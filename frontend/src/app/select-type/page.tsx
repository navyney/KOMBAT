...
export default function SelectMinions() {
    ...
    const minionsFromRedux = useSelector((state: RootState) => state.minionTypeSlice);
    ...
    useEffect(() => {
        ...
        client.onConnect = () => {
            ...
            client.publish({
                destination: "/app/join-select-minion-type",
    ...
    const toggleSelectMinion = (id: number) => {
        setSelectedMinions((prev) => {
            const updatedSelected = prev.includes(id) ? prev.filter((m) => m !== id) : [...prev, id];

            if (!updatedSelected.includes(id)) {
                const updatedMinionDetails = { ...minionDetails };
                delete updatedMinionDetails[id];
                setMinionDetails(updatedMinionDetails);
